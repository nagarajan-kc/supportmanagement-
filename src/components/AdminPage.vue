...
<tbody v-for="user in users" v-bind:key="user">
  ...
  <v-container>
    <v-row>
      ...
      <v-col>
        <v-btn class="btn btn-outline-dark"  v-on:click.prevent= "fetchData()" > fetchData </v-btn>
      </v-col>
    </v-row>
...
export default {
  ...
  methods:{
    ...
    fetchData() {
      axios.get('https://demoetenders.tn.nic.in/supportdora/getAllRequestListsForAdmin', 
      { 
        params: { "rowNo": 0 },
        headers:{
